...
/** The `kind` byte of a TSB/3 `EVENT` (§6.4.1), and the `type` field of the HTTP notification API.
  ...
  */
enum NotificationKind(val wire: String, val code: Int):
  case Follow extends NotificationKind("follow", 0x12)
  case Sub extends NotificationKind("sub", 0x13)
  case Gift extends NotificationKind("gift", 0x14)
  case Raid extends NotificationKind("raid", 0x15)
  case Chat extends NotificationKind("chat", 0x16)
  case Bits extends NotificationKind("bits", 0x17)
  case Info extends NotificationKind("info", 0x00)
