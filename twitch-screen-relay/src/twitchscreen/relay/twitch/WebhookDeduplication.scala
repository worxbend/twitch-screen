...
private[twitch] final class WebhookDeduplication(capacity: Int = 4096, lifetime: Duration = Duration.ofMinutes(10)):
  ...
  def claim(id: String, now: Instant): Either[Fail, Boolean] = claim(id, now, now.plus(lifetime))

  @tailrec final def claim(id: String, now: Instant, expiresAt: Instant): Either[Fail, Boolean] =
    ...
    else if entries.compareAndSet(before, live.updated(id, expiresAt)) then Right(true)
    else claim(id, now, expiresAt)
