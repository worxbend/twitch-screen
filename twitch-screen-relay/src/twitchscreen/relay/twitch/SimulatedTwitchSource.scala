...
private[twitch] object SimulatedTwitchSource:
  ...
  private val audience: Vector[RelayEvent] = Vector(
    ...
    RelayEvent.Subscribed("nightowl", tier = "1000", cumulativeMonths = 3),
    RelayEvent.BitsCheered("bitbaron", bits = 500),
    RelayEvent.SubscriptionGifted("generouspanda", count = 5, tier = "1000"),
  ...
  def start(config: TwitchConfig, bus: EventBus)(using Ox): TwitchSource =
    ...
    bus.publish(RelayEvent.StreamStarted(channel, "Building a round-display notifier", "Science & Technology"))

    cycle(config.simulation.interval, audience)(bus.publish)
    cycle(config.simulation.chatInterval, chatter)(line =>
      bus.publish(RelayEvent.ChatMessaged(chatters(line.length % chatters.size), line))
    )
