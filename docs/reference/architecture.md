...
- <a id="actor-mailboxes"></a>**Actor mailboxes.** Every relay actor is created with Ox's `Actor.create` and keeps Ox's default mailbox of 16 pending operations; the relay sets no `BufferCapacity` override. The four actors are DeviceHub, the Twitch credential transitions in [TwitchAuth](../../twitch-screen-relay/src/twitchscreen/relay/twitch/TwitchAuth.scala), [ChannelStateTracker](../../twitch-screen-relay/src/twitchscreen/relay/twitch/ChannelStateTracker.scala), and [TwitchRuntimeHealth](../../twitch-screen-relay/src/twitchscreen/relay/twitch/TwitchRuntimeHealth.scala). When a mailbox is full, `ask` blocks the calling thread until a slot frees: EventSub and other Twitch callback threads, the Helix poller, the token maintenance loop, HTTP handler threads, and device-session threads. That is backpressure, not dropping. The bound therefore relies on every actor operation being short: operations never block on a socket or a queue, and network calls to Twitch (token exchange, refresh, validation, revocation) run outside the actor. The one exception is credential persistence, which writes or deletes the small local token file inside the credential actor. `DeviceHub.connectedCount` reads an atomic counter and deliberately bypasses the mailbox.
...
| Relay actor mailbox (DeviceHub, credential transitions, channel state tracker, Twitch runtime health) | 16 pending operations (Ox default) | `ask` blocks the calling thread until a slot frees; nothing is dropped. See [Actor mailboxes](#actor-mailboxes). |
