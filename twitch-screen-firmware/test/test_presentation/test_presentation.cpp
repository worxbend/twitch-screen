...
bool isSevere(NotifyKind kind) { return kind == NotifyKind::Warning || kind == NotifyKind::Alert; }

// Every kind that is neither WARNING nor ALERT, plus unknown codes that fold to INFO.
std::array<NotifyKind, 12> routineKinds() {
  return {{NotifyKind::Info, NotifyKind::Message, NotifyKind::StreamStart, NotifyKind::StreamEnd,
...
void testCompactCounts(Checker &check) {
  struct Number {
    uint32_t value;
    const char *text;
  };
  const std::array<Number, 21> cases = {{{0, "0"}, {999, "999"}, {1000, "1.0K"},
  ...
  for (const auto &c : cases) {
    ...
    formatCount(text.data(), text.size(), c.value);
    check(strcmp(text.data(), c.text) == 0 && strnlen(text.data(), text.size()) <= 4,
          "bounded compact count");
  }
}

void testKindPresentation(Checker &check) {
...
void testLiveStatsWriteOnlyChanges(Checker &check) {
...
void testOfflineStatsWriteOfflineChips(Checker &check) {
  StatsShown shown; initStatsShown(shown);
...
void testHiddenStatsWriteNothing(Checker &check) {
  StatsShown shown; initStatsShown(shown);
  const StreamStats live = nextStats(1, 42, 1500, 7, 3, 12);
...
void testGroupSwitchRedraws(Checker &check) {
  StatsShown shown; initStatsShown(shown);
...
void testRoundingBucket(Checker &check) {
  StatsShown shown; initStatsShown(shown);
...
void testVisibilityRule(Checker &check) {
  for (int bits = 0; bits < 8; ++bits) {
    const bool linkUp = (bits & 1) != 0;
    const bool covered = (bits & 2) != 0;
    const bool live = (bits & 4) != 0;
    StatsGroup expected = StatsGroup::None;
    if (!covered && linkUp) expected = live ? StatsGroup::Live : StatsGroup::Offline;
...
void testReplaySkipsEntrance(Checker &check) {
  for (int code = 0; code < 256; ++code) {
    ...
    check(entranceFor(false, kind) == (isSevere(kind) ? Entrance::FlashThenSlide : Entrance::Slide),
  ...
  for (const NotifyKind kind : routineKinds())
    check(entranceFor(false, kind) == Entrance::Slide, "routine live cards only slide");
}

// K-118: only severe live cards pay for the full-panel ring pulse during the hold.
void testSevereRingPulse(Checker &check) {
  ...
  for (int code = 0; code < 256; ++code) {
    const NotifyKind kind = kindFromCode(static_cast<uint8_t>(code));
    check(ringPulses(entranceFor(false, kind)) == isSevere(kind),
          "only live warning/alert pulse the ring, any code");
    check(!ringPulses(entranceFor(true, kind)), "replay never pulses, any code");
  }
  for (const NotifyKind kind : routineKinds())
    check(!ringPulses(entranceFor(false, kind)), "routine live ring stays static");
}
...
int main() {
  Checker check;
  testCompactCounts(check);
  testKindPresentation(check);
  testLiveStatsWriteOnlyChanges(check);
  testOfflineStatsWriteOfflineChips(check);
  testHiddenStatsWriteNothing(check);
  testGroupSwitchRedraws(check);
  testRoundingBucket(check);
  testVisibilityRule(check);
  testReplaySkipsEntrance(check);
  testSevereRingPulse(check);
