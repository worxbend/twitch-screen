...
| LogBuffer resize in composition root | Retained intentionally: composition root owns applying deployment configuration to the single Logback-created global appender. Moving this one explicit startup call into unrelated telemetry would hide ownership without removing global state. |
| ConfigApi second config load | Main loads one HOCON snapshot, decodes it, and passes that same source to ConfigApi. |
| Sensitive.Empty sentinel | Retained intentionally as a masked external-config value. All enabled/disabled checks use `isSet`, and missing Basic/Bearer combinations are validated. Replacing every secret with Option would change HOCON default semantics without fixing a demonstrated failure. |
...
| Three RelayEvent classifications | Retained intentionally: summary text, activity category and metric routing answer different questions. The metrics tests prove observations/link transitions are excluded from audience-event counts; merging these classifications would conflate semantics. |
| Three bounded append expressions | Retained intentionally: the one-line Vector append/takeRight operation is simpler than another storage abstraction, and alert CAS state includes IDs/status transitions while logs support resize. |
