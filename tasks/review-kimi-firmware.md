...
| Every live card runs two full-panel effects | Fixed: routine cards use a slide; attention flash reserved for WARNING/ALERT; removed overlapping full-overlay opacity animation. Round 3 (K-118): option (a). The accent-ring opacity pulse (whole-object style opa, which forces a near-full-panel layered repaint every frame) now runs only on live WARNING/ALERT cards (`ringPulses(entranceFor(...))` in presentation.h, tested in test_presentation). Routine and replay rings are static at LV_OPA_COVER, so a routine card's only full-panel animation is the short slide-in/out. Owner still owes an on-device check (LVGL perf monitor, LV_USE_PERF_MONITOR, or a flush-area log in lv_port.cpp) that a routine card's hold no longer invalidates the full panel. |
