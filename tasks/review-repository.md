...
|---|---|
| REPO-01, REPO-02 | Fixed onboarding and both firmware skills for TSB/3, current native suites, relay simulation, refuse-newest admission and management authentication. Both skill validators pass. |
...
| RLY-04 | Dockerfile syntax check passes; full image build and container smoke recorded below after integration. |
| RLY-38, RLY-44 | Added `-Werror`, corrected JSON's build comment, removed hard-coded test counts, and configured CI compile/test/format gates. Baseline compiles cleanly with warnings fatal. |
...
| CAD-05 | Empty wall sample set now raises a useful domain-specific diagnostic before `min`. Normal 695-sample wall validation passes with 1.5 mm minimum. |
...
| CAD-08 | Shared preview size used by Blender and packaging. Existing preview dimensions retained; packaging validation checks the same constant. |
...
- GitHub CI execution, integrated container smoke and final judge results are
  pending; local commands are not represented as remote CI success.
